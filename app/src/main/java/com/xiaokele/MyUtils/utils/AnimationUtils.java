...
import com.QiyiLive.live.entity.AnimFrameConfig;
...
public class AnimationUtils {
    ...
    public static void animateRawManuallyFromFRAME(AnimFrameConfig.AnimFrame[] frames, final String defaultPath, final ImageView imageView, final Runnable onStart, final Runnable onComplete) {
        loadFromFrame(frames, defaultPath, imageView.getContext(), new OnDrawableLoadedListener() {
            @Override
            public void onDrawableLoaded(List<MyFrame> myFrames) {
                if (onStart != null) {
                    onStart.run();
    ...
    private static void loadFromFrame(final AnimFrameConfig.AnimFrame frames[], final String defaultPath, final Context context,
                                      final OnDrawableLoadedListener onDrawableLoadedListener) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                ...
                for (AnimFrameConfig.AnimFrame frame : frames) {
                    int duration = 1000;
                    byte[] bytes = null;

                    try {
                        bytes = IOUtils.toByteArray(new FileInputStream(new File(defaultPath + frame.getFrame())));
